...
public class Effect : MonoBehaviour
{
    ...
    public float Play()
    {
        ...
        if (light != null)
        {
            LightCurves lightCurves = light.GetComponent<LightCurves>();
            float lightCurveDuration = lightCurves.GraphScaleY;
            lifetime = Mathf.Max(lifetime, lightCurveDuration);
            lightCurves.Play();
        }
    ...
    public void Stop()
    {
        if (effectState == EffectState.PLAYING)
        {
            ...
            if (light != null && !loop)
            {
                light.GetComponent<LightCurves>().Stop();
            }
